...
TEST_F(AddTests, Add) {
    ...
    webnn::Operand c = builder.Add(a, b);
    webnn::Model model = utils::CreateModel(builder, {{"c", c}});
    webnn::Compilation compiledModel = utils::AwaitCompile(model);
    ...
    webnn::Input input = {aData.data(), aData.size() * sizeof(float)};
    webnn::Result result = utils::AwaitCompute(compiledModel, {{"a", input}}).Get("c");
...
TEST_F(AddTests, AddBroadcast) {
    ...
    addBroadcast->SetInput(
        {3, 4, 5}, {
                       ...
                       0.5992003,   -0.81047946, -2.2757835,  -0.21841764, 1.1650828,   -0.56927145,
                       1.9960726,   0.62048405,  0.10586528,  -1.0386543,  -1.9402571,  -2.0906122,
                       -0.4305259,  -1.2730165,  1.5639576,   0.53357494,  -0.8079486,  -0.06450062,
                       -0.7841324,  -0.24135855, 1.9275267,   0.4476717,   0.15467685,  -1.2363592,
                       -0.50745815, 0.03250425,  0.86344534,  -0.7938714,  1.1835734,   1.515135,
                       0.3092435,   -1.311751,   -0.6659017,  0.8815683,   -0.31157655, 0.57511795,
