...
ErrorScope *ErrorScope::GetParent() { return mParent.Get(); }

bool ErrorScope::IsRoot() const { return mIsRoot; }
...
void ErrorScope::RunNonRootCallback() {
  ...
  if (mCallback != nullptr) {
    ...
    mCallback(static_cast<WNNErrorType>(mErrorType), mErrorMessage.c_str(),
              mUserdata);
    mCallback = nullptr;
  }
}
...
void ErrorScope::UnlinkForShutdown() { UnlinkForShutdownImpl(this); }

// static
void ErrorScope::HandleErrorImpl(ErrorScope *scope, wnn::ErrorType type,
                                 const char *message) {
