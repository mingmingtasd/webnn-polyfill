...
${padding}}`;
  ...
  if (type.isObject && !type.isArray) {
    let unwrapType = getExplortDeclarationName(type.nativeType);
    out += `\n${padding}${output.name}.${member.name} = Napi::ObjectWrap<${unwrapType}>::Unwrap(${input.name}.Get("${member.name}").As<Napi::Object>())->GetOperand();`;
...
${padding}  ${type.nativeType} value = Napi::ObjectWrap<${unwrapType}>::Unwrap(item)->GetOperand();
